...
def version_resources() -> None:
    ...
    console.print(f"[bold blue]azp-res[/bold blue] version [bold green]{__version__}[/bold green]")
...
def view_entra_history(
    ...
    try:
        ...
        console.print(
            f"[bold blue]Fetching Entra role activation history (last {days} days)...[/bold blue]"
        )
...
def activate_group_role(
    ...
    try:
        ...
        console.print(
            "[bold green]✓ Group assignment activation requested successfully![/bold green]"
        )
        console.print(f"[dim]Request ID: {result.get('id', 'N/A')}[/dim]")
