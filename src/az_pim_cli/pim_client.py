...
class PIMClient:
    ...
    def list_group_assignments(
        ...
        url = (
            f"{self.GRAPH_API_BETA}/identityGovernance/privilegedAccess/group/eligibilitySchedules"
        )
        ...
        while True:
            ...
            if self.verbose:
                print(
                    f"[DEBUG] Retrieved {len(values)} group assignments (total: {len(all_results)})"
                )
